...
class FormGenerator extends AbstractController
{
     ...
     */
    public  function  processForeignApplicant(Request $request,UserPasswordEncoderInterface $encoder, MailerInterface $mailer){
        ...
        if(!empty($phone) && !empty($name) && !empty($country) && !empty($email)) {
            ...
            $email_message = "
                       <p>Takoradi Technical University - Admissions</p>
            ...
            $emails = (new Email())
                ->from('admissions@ttu.edu.gh')
                ->to($email)
                ->cc('ipelo@ttu.edu.gh')
                ->bcc('tpconnect@ttu.edu.gh')
                //->replyTo('fabien@example.com')
                ->priority(Email::PRIORITY_HIGH)
                ->subject('Takoradi Technical University-Ghana')
                ->html($email_message)
            ;
