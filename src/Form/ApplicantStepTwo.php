...
use App\Repository\SchoolRepository as former;
...
class ApplicantStepTwo extends AbstractType
{
    ...
    public function buildForm(FormBuilderInterface $builder, array $options)
    {
        ...
        $builder
            ...
            ->add('programmeID1', EntityType::class, [
                ...
                'query_builder' => function (prepo $repository) use($type)
                {
                    ...
                    elseif ($type=="BTECH" ||  $type=="DEGREE" || $type=="DGREE"){
                        $type="BTECH";
                        return $repository->createQueryBuilder('p')
                            ->where('p.running = ?1')
                            ->andWhere('p.type = ?2')
            ...
            ->add('formerSchool', EntityType::class, [
                ...
                'class' => School::class,
                'query_builder' => function (former $repository) use($type)
                {
        ...
        if ($options['type']=="BTECH" || $options['type']=="MTECH" || $options['type']=="ACCELERATED_BTECH" ||  $options['type']=="ACCELERATED_HND" ) {
